...
const StatusBadge = ({ isConnected }) => (
  ...
  >
    <span className="relative flex h-3 w-3">
      <span
        className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-60 ${isConnected ? "bg-green-500" : "bg-red-500"}`}
      ></span>
      <span
        className={`relative inline-flex rounded-full h-3 w-3 ${isConnected ? "bg-green-600" : "bg-red-600"}`}
      ></span>
    </span>
...
export default function MainLayout({ children, activeTab, setActiveTab }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
      {/* ── SIDEBAR DESKTOP ── */}
      <aside className="hidden md:flex fixed inset-y-0 left-0 w-56 flex-col bg-white border-r border-slate-200 pt-6 pb-5 gap-5 z-30 shadow-sm">
        <SidebarContent
          activeTab={activeTab}
          setActiveTab={setActiveTab}
          setSidebarOpen={setSidebarOpen}
